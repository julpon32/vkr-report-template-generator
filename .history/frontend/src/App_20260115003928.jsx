...
export default function App() {
  ...
  const [historyShown, setHistoryShown] = useState(0); // сколько старых записей показывать
  const [templates, setTemplates] = useState([]);
  const [profileName, setProfileName] = useState("");
  const [analyzeMode, setAnalyzeMode] = useState("rules");
  ...
  async function analyze() {
    ...
    try {
      ...
      const res = await fetch(`${API}/api/analyze?mode=${encodeURIComponent(analyzeMode)}`, {
        method: "POST",
        body: form,
      });
      ...
      <div className="container">
        <div className="hero">
          <div className="heroTop">
            <div>
              <h1 className="heroTitle">Генератор шаблонов отчётов</h1>
              <p className="heroSubtitle">
                Веб-приложение для автоматизированного построения шаблонов отчётов на основе анализа требований
                к оформлению. Поддерживаются .docx, .txt, .pdf.
              </p>
            </div>

            {/* 
            <div className="heroMeta">
              <span className="pill">Backend: {API.replace("http://", "")}</span>
              <span className="pill">Frontend: localhost:5173</span>
            </div> */}
          </div>
        </div>
        ...
        <div className="grid">
            ...
            <div className="card">
              ...
              <div className="row">
                ...
                <select
                  ...
                  style={{ maxWidth: 220 }}
                >
                  <option value="rules">Анализ: правила</option>
                  <option value="llm">Анализ: ИИ (LLM)</option>
                  <option value="hybrid">Анализ: гибрид</option>
                </select>
          ...
          <div>
            ...
            <div className="card" style={{ marginTop: 14 }}>
              ...
              {history.length === 0 ? (
                <div className="small">История пока пустая.</div>
              ) : (
                <>
                  {/* Всегда показываем самый свежий элемент */}
                  <div className="item">
                    <div className="itemTitle">{history[0]?.filename}</div>
                    <p className="itemMeta">Дата: {formatTs(history[0]?.created_at)}</p>
                    <div className="itemActions">
                      <button className="btn" onClick={() => applyRules(history[0]?.rules)} disabled={loading}>
                        Применить правила
                      </button>
                    </div>
                  </div>

                  {/* Старые элементы: по 5 за раз */}
                  <div style={{ marginTop: 10 }} className="row">
                    {history.length > 1 && historyShown === 0 && (
                      <button className="btn" onClick={() => setHistoryShown(5)} disabled={loading}>
                        Показать историю
                      </button>
                    )}
                    ...
                    {history.length > 1 && historyShown > 0 && (
                      <>
                        <button className="btn" onClick={showMoreHistory} disabled={loading}>
                          Показать ещё
                        </button>
                        <button className="btn" onClick={collapseHistory} disabled={loading}>
                          Свернуть
                        </button>
                      </>
                    )}

                    {history.length <= 1 && <div className="small">Больше записей нет.</div>}
                  </div>
                  ...
                  {historyShown > 0 && (
                    <>
                      ...
                      <div className="sideList">
                        {history.slice(1, 1 + historyShown).map((h) => (
                      ...
                      <div className="spacer" />
                      <div className="small">
                        Показано: {Math.min(history.length, 1 + historyShown)} из {history.length}
                      </div>
                    </>
                  )}
                </>

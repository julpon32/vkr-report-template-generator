...
def apply_rules(text: str, fragments_override: Optional[List[str]] = None) -> RequirementsModel:
    ...
    def _is_font_candidate(frag: str) -> bool:
        lf = frag.lower()
        return "шрифт" in lf and ("times new roman" in lf or "arial" in lf or "calibri" in lf)

    best_font = _pick_best(fragments, _is_font_candidate, _score_font_fragment)

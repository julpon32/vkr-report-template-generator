...
def extract_rules(text: str) -> ExtractedRules:
    # 1) базовая модель (правила)
    model = apply_rules(text)

    # 2) ML протокол: классифицируем фрагменты
    t = normalize_text(text)
    fragments = split_fragments(t)

    ml_tags = predict_labels(fragments)

    # 3) Поддержка доказательств: сохраняем краткий протокол для ВКР/интерфейса
    # покажем первые 30 фрагментов (чтобы не раздувать ответ)
    ml_preview = ml_tags[:30]

    # 4) Фильтрация evidence (опционально, но полезно):
    # если evidence содержит явный мусор, можно оставить как есть, но мы пометим.

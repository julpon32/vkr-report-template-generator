...
def extract_rules(text: str) -> ExtractedRules:
    ...
    try:
        ...
        ml_note = "ML: TF-IDF (word+char ngrams) + LinearSVC (calibrated); используется для выделения фрагментов-требований."
    ...
    evidence["_ml_filtered_fragments_count"] = len(kept) if kept else 0
    ...
    rules = ExtractedRules(
        ...
        source_summary="Извлечение через rule-engine + ML-фильтрация фрагментов (TF-IDF + LinearSVC calibrated)",
...
# cd ~/Desktop/vkr-template-app/backend
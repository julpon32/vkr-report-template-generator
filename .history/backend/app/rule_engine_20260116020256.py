...
def _extract_mm_after(fragment: str, side_keywords: List[str]) -> Optional[int]:
    lf = fragment.lower()
    for kw in side_keywords:
        m = re.search(rf"{kw}\w*\s*(?:пол\w*)?\s*[:\-]?\s*(\d{{1,3}})\s*(?:мм|миллимет\w*)?", lf)
        if m:
            try:
...
def _extend_with_bigrams(fragments: List[str]) -> List[str]:
    out = list(fragments)
    for i in range(len(fragments) - 1):
        a = fragments[i].strip()
        b = fragments[i + 1].strip()
        if not a or not b:
            continue
        combo = f"{a} {b}"
        if len(combo) <= 450:
...
def apply_rules(text: str, fragments_override: Optional[List[str]] = None) -> RequirementsModel:
    ...
    fragments_ext = _extend_with_bigrams(fragments)
    # -------- ШРИФТ ДОКУМЕНТА (основной текст) --------
    def _is_doc_font_candidate(frag: str) -> bool:
        lf = frag.lower()

        # 1) должен быть один из поддерживаемых шрифтов
        has_font = ("times new roman" in lf) or ("arial" in lf) or ("calibri" in lf)
        if not has_font:
            return False

        # 2) принимаем не только "шрифт", но и "кегль"/"кегл" (типичный случай: "Calibri, кегль 12")
        has_font_context = ("шрифт" in lf) or ("кегл" in lf) or ("кегль" in lf)
        if not has_font_context:
            return False

        # 3) НЕ берём заголовки и нумерацию как шрифт основного текста
    ...
    def _score_doc_font_fragment(frag: str) -> int:
        ...
        if "times new roman" in lf or "arial" in lf or "calibri" in lf:
            score += 2
        if ("кегл" in lf or "кегль" in lf) and ("times new roman" in lf or "arial" in lf or "calibri" in lf):
            score += 4
        return score

    best_doc_font, best_doc_font_score = _pick_best_with_score(
        fragments_ext, _is_doc_font_candidate, _score_doc_font_fragment
    )
    ...
    best_doc_fontsize, best_doc_fontsize_score = _pick_best_with_score(fragments_ext, _is_doc_fontsize_candidate, _score_doc_fontsize_fragment)
    ...
    best_page_font, best_page_font_score = _pick_best_with_score(
        fragments_ext, _is_page_font_candidate, _score_font_fragment
    )
    ...
    best_spacing = _pick_best(fragments_ext, _is_spacing_candidate, _score_textbody_fragment)
    ...
    best_indent = _pick_best(fragments_ext, _is_indent_candidate, _score_textbody_fragment)
    ...
    for frag in fragments:
        lf = frag.lower()
        if not (("пол" in lf) and ("мм" in lf or "миллимет" in lf)):
            continue

        v = _extract_mm_after(frag, ["лев"])
        if v and 5 <= v <= 60:
            model.margins.left_mm = v
            evidence["margin_left_mm"] = frag

        v = _extract_mm_after(frag, ["прав"])
        if v and 5 <= v <= 60:
            model.margins.right_mm = v
            evidence["margin_right_mm"] = frag

        v = _extract_mm_after(frag, ["верх"])
        if v and 5 <= v <= 60:
            model.margins.top_mm = v
            evidence["margin_top_mm"] = frag

        v = _extract_mm_after(frag, ["ниж"])
        if v and 5 <= v <= 60:
    ...
    for frag in fragments:
        ...
        if "номер" in lf and ("страниц" in lf or "нумерац" in lf or "колонтитул" in lf):
            if "внизу" in lf and "цент" in lf:
                model.page_numbering.position = "bottom_center"
                evidence["page_number_position"] = frag
            elif "внизу" in lf and "справ" in lf:
                model.page_numbering.position = "bottom_right"
                evidence["page_number_position"] = frag
            elif "внизу" in lf and "лев" in lf:
                model.page_numbering.position = "bottom_left"
                evidence["page_number_position"] = frag

        for frag2 in fragments_ext:
            lf2 = frag2.lower()
            if ("номер" in lf2 and "страниц" in lf2 and "шрифт" in lf2) or ("размер шрифта номера страницы" in lf2):
                n = _extract_first_number(frag2)
                if n and 6 <= n <= 30:
                    model.page_numbering.font_size_pt = int(n)
                    evidence["page_number_font_size_pt"] = frag2
                    break
